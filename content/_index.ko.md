+++
draft = false

title = "임정섭"
date = 2025-01-08
description = "고려대학교 대학원생 Jeongseop Lim (임정섭)의 개인 프로필 페이지이다. 연구 관심 분야는 프로그래밍 언어와 소프트웨어 분석이다."

[params.header.number]
enable = false
+++

{{< style "text-align:center;" >}} 석사과정 학생

[소프트웨어 분석 연구실](https://prl.korea.ac.kr) \
[고려대학교](https://korea.ac.kr) [정보대학](https://info.korea.ac.kr) [컴퓨터학과](https://cs.korea.ac.kr) {{< /style >}}

## 소개

고려대학교 대학원 컴퓨터학과 석사과정에 재학 중인 임정섭입니다. 소프트웨어 분석 연구실에 소속되어 오학주 교수님의 지도를 받으며, 현실의 대규모 소프트웨어 시스템에서 활용 가능한 실용적인 프로그램 분석 기법을 목표로 연구하고 있습니다.

이전에는 금융 스타트업에서 Python 백엔드 개발자로 근무하며 대출 및 투자 프로세스를 자동화하는 시스템을 개발했습니다. 특히 유지보수하기 쉬운 소프트웨어를 만드는 일에 가치를 두고, 탄탄한 아키텍처 설계와 문서화 그리고 건강한 개발 문화를 구축하는 데 힘썼습니다.

_연구 관심 분야_: 프로그래밍 언어 | 정적 프로그램 분석 | Python 언어 및 생태계 | 자동 소프트웨어 테스팅 (동적 프로그램 분석)

## 학력

- [**고려대학교**](https://korea.ac.kr) [컴퓨터학과](https://cs.korea.ac.kr) **석사** 재학 ({{< period from="2026-03" >}})
  - [소프트웨어 분석 연구실](https://prl.korea.ac.kr)(지도교수: [오학주](https://prl.korea.ac.kr/members/hakjoo-oh))

- [**고려대학교**](https://korea.ac.kr) [산업경영공학](https://ie.korea.ac.kr), [컴퓨터학](https://cs.korea.ac.kr)(이중전공) **학사** 졸업 ({{< period from="2020-03" to="2026-02" >}})
  - 학점: 4.46 / 4.5

- [영재학교 **대구과학고등학교**](https://dshs.dge.hs.kr) 졸업 ({{< period from="2016-02" to="2019-01" >}})

## 경력

- [고려대학교](https://korea.ac.kr) [소프트웨어 분석 연구실](https://prl.korea.ac.kr) **학부 연구생** ({{< period from="2025-01" to="2026-02" >}})

- [(주)에잇퍼센트](https://8percent.kr) **백엔드 개발자** ({{< period from="2022-08" to="2024-08" >}}) \
  산업기능요원 복무
  - 백엔드 개발자 온보딩 프로그램 수립 [{{< fa-icon solid link >}}](https://8percent.github.io/2024-07-14/%EB%B0%B1%EC%97%94%EB%93%9C-%EC%98%A8%EB%B3%B4%EB%94%A9-%EC%88%98%EB%A6%BD)
  - 레거시 대출 신청 시스템 재설계
  - Python 백엔드 프레임워크 Django 버전을 3.2에서 4.1로 업그레이드
  - Firebase Cloud Messaging을 이용한 푸시 알림 시스템 개발 및 관리

<!-- ## 출판물 -->

## 수상 및 장학

- **관정재단 장학생**, [관정재단](https://www.ikef.or.kr) ({{< period from="2026년 1학기" >}})

- **졸업특대생**, [고려대학교](https://korea.ac.kr) ({{< date "2026-02" >}}) \
  공과대학 수석 졸업

- **학장상**, [고려대학교](https://korea.ac.kr) ({{< date "2025-02" >}}, {{< date "2022-02" >}}) \
  학기별 성적우수상

- **국가우수장학금(이공계)**, [한국장학재단](https://www.kosaf.go.kr) ({{< period from="2022년 1학기" to="2025년 2학기" >}}) \
  4학기간 전액 등록금 수혜

- **제8기 S&T장학생**, [운해장학재단](https://woonhaefoundation.org) ({{< period from="2021년 1학기" to="2021년 2학기" >}})

- **가헌신도재단 장학생**, [가헌신도재단](https://www.gaheon-sindoh.or.kr) ({{< date "2020년 2학기" >}})
