...
*시스템 분석*은 현실의 복잡한 시스템을 약속된 형식으로 요약, 즉 모델링하는 행위이다. 시스템을 편리하게 이해하고, 개발된 최적화 도구를 바로 적용하는 데 필요하다. 그럼 *최적화*는 무엇일까? 같은 입력에 대해 더 나은 성능을 얻는 것이다. 주어진 분석 결과에서 최적의 성능을 내는 결정을 찾는 절차(알고리즘)를 이른다. 입력, 출력, 그리고 성능의 정의는 시스템에 달려 있다. 시스템이 다양하니 분석과 최적화 기법도 그만큼 갈래가 나뉜다. 아래 그림은 산업공학의 요소가 문제 해결 과정에서 어떤 관계를 맺는지 나타낸다.
...
- *생산계획*에서 생산량과 일정을 결정하는 데 필요한 기간별 수요량을 예측하는 데는 *예측애널리틱스*에서 배운 시계열 분석 기법이나 *다변량 분석*에서 배운 AI 기반 회귀 분석 기법이 활용된다.
